...
const RevenueSharingWidget = () => {
  ...
  if (loading) {
    return <div className="bg-gray-900 rounded-2xl shadow-sm border border-gray-800 p-8 h-full animate-pulse"></div>;
  }

  return (
    <div className="bg-gray-900 rounded-2xl shadow-sm border border-gray-800 p-8 h-full">
      <h2 className="text-2xl font-bold text-white mb-6">Revenue Sharing</h2>
      
      <div className="bg-indigo-500/10 rounded-2xl p-6 mb-6">
        <div className="flex justify-between items-center mb-4">
          <p className="text-indigo-300 font-semibold text-sm uppercase">Your Earnings</p>
        ...
        <p className="text-4xl font-extrabold text-white">${revenue?.user_share?.toFixed(2) || '0.00'}</p>
        <p className="text-indigo-300 text-sm mt-2">from ${revenue?.total_platform_revenue?.toLocaleString() || '0'} total platform revenue</p>
      </div>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <div className="p-4 bg-gray-900 rounded-xl">
          <p className="text-xs text-gray-400 font-medium uppercase mb-1">Next Drop</p>
          <p className="font-bold text-white">Every 24h</p>
        </div>
        <div className="p-4 bg-gray-900 rounded-xl">
          <p className="text-xs text-gray-400 font-medium uppercase mb-1">Share Rate</p>
          <p className="font-bold text-white">10.0%</p>
        </div>
      </div>
      ...
      <button 
        ...
        className={`w-full py-4 rounded-xl font-bold text-white transition-all shadow-md active:scale-95 ${
          claiming || !revenue || revenue.user_share <= 0 ? 'bg-indigo-600 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'
        }`}
      >
