  ...
  return (
    <>
      ...
      <div className="space-y-6">
        {/* Search + header */}
        <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-base font-bold text-white">On-Chain Activity</h2>
            <p className="text-xs text-gray-500 mt-0.5">Verifiable records of all AI-optimized transactions</p>
          </div>
          <div className="relative">
            ...
            <input type="text" placeholder="Search by hash or route..." value={search} onChange={e => setSearch(e.target.value)}
              className="pl-9 pr-4 py-2.5 border border-gray-700 rounded-xl text-sm text-gray-500 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 w-64"/>
          </div>
        </div>
        ...
        <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
              ...
              <tbody>
                {filtered.length > 0 ? filtered.map(tx => (
                  <tr key={tx.id} className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm font-semibold text-white">${Number(tx.amount).toLocaleString()}</td>
          ...
          <div className="px-6 py-4 border-t border-gray-800 flex items-center justify-between">
            <p className="text-xs text-gray-500">Showing {filtered.length} of {transactions.length} records</p>
